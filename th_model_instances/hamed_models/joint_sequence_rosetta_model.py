...
def joint_network(max_residues, padding):
    sequence_inputs = Input(shape=(23, max_residues + 2 + 2 * padding, 1))  # 22 amino acids plus null/beginning/end
    rosetta_inputs = Input(shape=(113,))

    # what is the point of this next line? --> Ask Jed
    sequence_inputs = Lambda(lambda x: x[:, :23, :, :])(sequence_inputs)
    ...
    rosetta_model = Dropout(0.019414354060286951)(rosetta_model)
    # rosetta_model.add(Dense(units=1))
    ...
    merged_model = Model(inputs=[sequence_inputs, rosetta_inputs], outputs=output_layer)
    ...
    # merged_model.fit([train1, train2], y_train, nb_epoch=10, batch_size=28)

    th_model = KerasJointRegression(model=sequence_model, model_description='Joint Model', batch_size=128, epochs=25)
    return th_model
